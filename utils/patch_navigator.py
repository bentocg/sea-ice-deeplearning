...
class PatchNavigator:
    def __init__(self, scenes_dir: str, patch_size: int, scn_res: tuple, out_dir: str,
        ...
        self.curr_mask = None
        self.pos_mask = np.ones([self.patch_size, self.patch_size])
        self.neg_mask = np.zeros([self.patch_size, self.patch_size])
    ...
    def write_patch(self, label):
        print(self.curr_patch.shape)
        cv2.imwrite(f"{self.out_dir}/x/{os.path.basename(self.input_scn)}_{self.i}_{self.j}.tif", self.curr_patch)
        if label == 'keep':
            cv2.imwrite(f"{self.out_dir}/y/{os.path.basename(self.input_scn)}_{self.i}_{self.j}.tif", self.curr_mask)
        if label == 'positive':
            cv2.imwrite(f"{self.out_dir}/y/{os.path.basename(self.input_scn)}_{self.i}_{self.j}.tif", self.pos_mask)
        elif label == 'negative':
            cv2.imwrite(f"{self.out_dir}/y/{os.path.basename(self.input_scn)}_{self.i}_{self.j}.tif", self.neg_mask)
        self.next_cell()
